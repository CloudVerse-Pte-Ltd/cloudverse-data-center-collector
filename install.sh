...
IMAGE_DEFAULT="ghcr.io/cloudverse-pte-ltd/cloudverse-data-center-collector@sha256:612f6ae6e2158048bf3cd5a90363aecd296c526a67ee6ee5b67e8ba6931ca85c"
CONTROL_PLANE_URL=""
ORG_ID=""
INTEGRATION_ID=""
PROVIDER=""
...
usage() {
  echo "Usage: $0 --control-plane-url URL --org-id ID --integration-id ID --provider VSPHERE|OPENSHIFT_VIRTUALIZATION --scale-class S|M|L|XL --enrollment-token TOKEN" >&2
  exit 2
}
...
if [ -z "$CONTROL_PLANE_URL" ] || [ -z "$ORG_ID" ] ||
  ...
  [ -z "$SCALE_CLASS" ] || [ -z "$ENROLLMENT_TOKEN" ]; then
  usage
fi
...
case "$SCALE_CLASS" in S|M|L|XL) ;; *) usage ;; esac
case "$PROVIDER" in
  VSPHERE) ;;
  OPENSHIFT_VIRTUALIZATION)
    ...
    exec "$OPENSHIFT_INSTALLER" --control-plane-url "$CONTROL_PLANE_URL" --org-id "$ORG_ID" --integration-id "$INTEGRATION_ID" --scale-class "$SCALE_CLASS" --enrollment-token "$ENROLLMENT_TOKEN"
    ;;
  *) usage ;;
esac
...
docker run -d --name cloudverse-data-center-collector --restart unless-stopped \
  ...
  -e COLLECTOR_CONTROL_PLANE_URL="$CONTROL_PLANE_URL" \
  -e COLLECTOR_ORG_ID="$ORG_ID" -e COLLECTOR_INTEGRATION_ID="$INTEGRATION_ID" \
  -e COLLECTOR_PROVIDER=VSPHERE -e COLLECTOR_SCALE_CLASS="$SCALE_CLASS" \
  -e COLLECTOR_ENROLLMENT_TOKEN_FILE=/var/lib/cloudverse/identity/enrollment-token \
  -e COLLECTOR_PROVIDER_CONFIG_FILE=/var/lib/cloudverse/config/provider.json \
  -e COLLECTOR_ALLOWED_HOSTS="$API_HOST" \
  -e NODE_EXTRA_CA_CERTS=/var/lib/cloudverse/config/provider-ca.pem \
  -v "$STATE_ROOT/identity:/var/lib/cloudverse/identity" \
  -v "$STATE_ROOT/spool:/var/lib/cloudverse/spool" \
  -v "$STATE_ROOT/config:/var/lib/cloudverse/config:ro" \
  -v "$STATE_ROOT/run:/var/run/cloudverse" \
  "$IMAGE"
