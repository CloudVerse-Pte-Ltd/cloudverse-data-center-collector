  ...
  const resources = input.result.objects.flatMap((object): VCenterInventoryResource[] => {
    ...
    return [{
      ...
      attributes: {
        ...
        instanceUuid: normalizedUuid(object.properties['config.instanceUuid']),
        biosUuid: normalizedUuid(object.properties['config.uuid']),
        template: resourceKind === 'TEMPLATE',
        datastoreUrl: propertyText(object.properties['summary.url']) || undefined,
        datastoreType: propertyText(object.properties['summary.type']) || undefined,
